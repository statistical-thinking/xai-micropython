...
# Target group
XAI-MICROPYTHON draws on well-known exercise data sets and common methods to give (academic) teachers a quick start in designing their own teaching materials. In addition, XAI-MICROPYTHON can of course also be used by self-learners. In higher education teaching, XAI-MICROPYTHON has been used for assignments in which students extend the provided implementations, for example by translating additional clustering algorithms from their mathematical formulation into MicroPython code, modifying the architecture of neural networks, or implementing additional activation functions. Educators can adopt the framework as a complete learning sequence or select individual examples according to their teaching objectives, using either the browser-based environment or MicroPython-compatible microcontrollers.
